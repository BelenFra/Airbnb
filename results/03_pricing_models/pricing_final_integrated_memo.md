...
**Status:** Figures and numbered tables cited below are reproducible from the artefacts **committed alongside this memo under `results/03_pricing_models/`** (same-directory paths use `./`). No models were retrained for this memo; values were taken from earlier project outputs at memo creation.
...
| **Pricing optimization preparation (descriptive)** | Describe joint behavior of price, **`estimated_occupancy_l365d`**, **`annual_revenue_proxy`** | Filters on valid positive price / occupancy range; correlations; means by **`City`** and **`room_type`**; equal-count **price tertiles** | `./02_correlation_pearson.csv`; `./03_summary_by_city.csv`; `./04_summary_by_room_type.csv`; `./05_price_bands_tertiles_overall.csv`; `./05b_price_band_cutpoints_global.csv` |
...
Split sizes (training pipeline): **62,100 train / 20,700 validation / 20,700 test** (**≈ 60 % / 20 % / 20 %**), consistent with stratified partitioning in the retained training scripts.
...
*(Source: `./model_comparison_table.csv`; values rounded here for readability.)*
...
**Entire-home minus private-room mean-log-price gap (see `./summary_city_entire_minus_private_gap.csv`):**
...
Supporting QA from amenities parsing (parse reliability metrics in **`./amenities_parse_summary_metrics.csv`**): strict JSON-array parse succeeded on **≈99.97%** of master rows (**34 failures**)—engineered amenity binaries are statistically usable.
...
Pearson matrix from **`./02_correlation_pearson.csv`:**
...
As summarized in **the companion descriptive-pricing documentation for this project**, **`log(price)` versus estimated occupancy** is about **−0.16** descriptively—**softer cross-sectional “price ↔ fewer estimated nights”.**
...
_Files in this GitHub bundle live under **`results/03_pricing_models/`** (paths below are relative to that folder). Narrative predecessors are not replicated in this upload._

| Content | Primary files in this handoff |
|---------|-------------------------------|
| This integrated memo | `./pricing_final_integrated_memo.md` |
| Model leaderboard | `./model_comparison_table.csv` |
| Permutation-importance backbone | `./feature_importance_permutation_validation.csv` |
| City entire-home vs private-room gap (EDA) | `./summary_city_entire_minus_private_gap.csv` |
| Amenities parsing QA metrics | `./amenities_parse_summary_metrics.csv` |
