...
> Owner: Yu Wang (calendar lead).
> Last updated: 2026-04-26.
> Related document: `results/calendars/calendar_cleaning_decisions.md` (cleaning rules).
> A Chinese-language copy of this document is kept at `calendar_dataset_README_CN.md`.

## 1. Which file should I use?

| Use case | File | Rows / size |
| --- | --- | --- |
| Listing-level investment comparison or modeling | `data/processed/calendars/all_cities_listing_occupancy.csv` | 132,677 rows / 16 MB |
| Single-city occupancy analysis | `data/processed/calendars/<city>_listing_occupancy.csv` | see table below |
| Multi-city day-level time series (seasonality, weekly patterns) | `data/processed/calendars/all_cities_calendar_cleaned.csv` | ~48.4M rows / 2.3 GB |
| Single-city day-level time series | `data/processed/calendars/<city>_calendar_cleaned.csv` | see table below |
| Cleaning audit / decision tracking | `results/calendars/calendars_cleaning_audit.csv` | 5 rows |

> **Recommendation**: unless you specifically need seasonality, **default to `all_cities_listing_occupancy.csv`**. It collapses the 365-day calendar to one row per listing and already joins in `listing_price` from `listings.csv`.
...
listings = pd.read_csv('data/processed/listings/all_cities_listings_cleaned.csv')  # Belu's deliverable
...
- [ ] Re-run: the script is idempotent — delete `data/processed/calendars/` and run `python scripts/cleaning/calendars/run_full_calendar_cleaning.py` (~22 minutes).
...
Inputs: `data/Term Project/<City>/{calendar.csv, listings.csv}`.
Outputs: `data/processed/calendars/`, plus `results/calendars/calendars_cleaning_audit.csv`.
